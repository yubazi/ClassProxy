...
package com.bug.dexmaker.dx;

import com.bug.dexmaker.dx.rop.code.BasicBlockList;
import com.bug.dexmaker.dx.rop.code.Insn;
import com.bug.dexmaker.dx.rop.code.PlainCstInsn;
import com.bug.dexmaker.dx.rop.code.PlainInsn;
import com.bug.dexmaker.dx.rop.code.RegisterSpecList;
import com.bug.dexmaker.dx.rop.code.Rop;
import com.bug.dexmaker.dx.rop.code.Rops;
import com.bug.dexmaker.dx.rop.code.SourcePosition;
import com.bug.dexmaker.dx.rop.code.ThrowingCstInsn;
import com.bug.dexmaker.dx.rop.code.ThrowingInsn;
import com.bug.dexmaker.dx.rop.cst.CstInteger;
...
import static com.bug.dexmaker.dx.rop.code.Rop.BRANCH_GOTO;
import static com.bug.dexmaker.dx.rop.code.Rop.BRANCH_NONE;
import static com.bug.dexmaker.dx.rop.code.Rop.BRANCH_RETURN;
import static com.bug.dexmaker.dx.rop.type.Type.BT_BYTE;
import static com.bug.dexmaker.dx.rop.type.Type.BT_CHAR;
import static com.bug.dexmaker.dx.rop.type.Type.BT_INT;
import static com.bug.dexmaker.dx.rop.type.Type.BT_SHORT;
...
public final class Code {
    ...
    public void cast(Local<?> target, Local<?> source) {
        cast(target, source, target.type);
    }

    public void cast(Local<?> target, Local<?> source, TypeId<?> type) {
        if (source.getType().ropType.isReference()) {
            addInstruction(new ThrowingCstInsn(Rops.CHECK_CAST, sourcePosition,
                    RegisterSpecList.make(source.spec()), catches, type.constant));
            moveResult(target, true);
        } else {
            addInstruction(new PlainInsn(getCastRop(source.type.ropType, type.ropType),
                    sourcePosition, target.spec(), source.spec()));
        }
    }

    public void castType(Local<?> source, TypeId<?> type) {
        addInstruction(new ThrowingCstInsn(Rops.CHECK_CAST, sourcePosition, RegisterSpecList.make(source.spec()), catches, type.constant));
        moveResult(source, true);
    }

    // instructions: arrays

    private Rop getCastRop(com.bug.dexmaker.dx.rop.type.Type sourceType,
                           com.bug.dexmaker.dx.rop.type.Type targetType) {
