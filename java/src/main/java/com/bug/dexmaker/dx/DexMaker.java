...
import com.bug.dexmaker.dx.dex.DexFormat;
...
import com.bug.dexmaker.dx.dex.code.RopTranslator;
import com.bug.dexmaker.dx.dex.file.ClassDefItem;
import com.bug.dexmaker.dx.dex.file.DexFile;
import com.bug.dexmaker.dx.dex.file.EncodedField;
import com.bug.dexmaker.dx.dex.file.EncodedMethod;
import com.bug.dexmaker.dx.rop.code.AccessFlags;
import com.bug.dexmaker.dx.rop.code.LocalVariableInfo;
import com.bug.dexmaker.dx.rop.code.RopMethod;
...
import static com.bug.dexmaker.dx.rop.code.AccessFlags.ACC_CONSTRUCTOR;
...
public final class DexMaker {
    ...
    public void declare(TypeId<?> type, String sourceFile, int flags,
        ...
        int supportedFlags = Modifier.PUBLIC | Modifier.PRIVATE | Modifier.PROTECTED | Modifier.FINAL | Modifier.ABSTRACT
    ...
    public ClassLoader generateAndLoad(ClassLoader parent, File dexCache, boolean restart) throws IOException {
        return generateAndLoad(parent, dexCache, generateFileName(), restart);
    }
     ...
     */
    public ClassLoader generateAndLoad(ClassLoader parent, File dexCache, String dexFileName, boolean restart) throws IOException {
        byte[] dex = generate();
        ...
        File result = new File(dexCache, (dexFileName == null || dexFileName.isEmpty()) ? generateFileName() : dexFileName);
        // Check that the file exists. If it does, return a DexClassLoader and skip all
        // the dex bytecode generation.
        if (result.exists() && !restart) {
        ...
        JarOutputStream jarOut = new JarOutputStream(new BufferedOutputStream(new FileOutputStream(result)));
        try {
